...
class App:
    def __init__(self, root, items_by_slot):
        ...
        self.selected_food = tk.StringVar(
            value=list(self.foods.keys())[0] if self.foods else ""
        )
    ...
    def build_ui(self):
        ...
        ttk.Combobox(
            self.root,
            textvariable=self.selected_food,
            values=list(self.foods.keys())
        ).pack()
    ...
    def run(self):
        ...
        for slot, items in self.items_by_slot.items():
            ...
            # sort properly (fix for augmented gear issue)
            valid = sorted(
                valid,
                key=lambda x: (x["ilvl"], sum(x["stats"].values())),
                reverse=True
            )

            # limit to top 3 per slot (performance)
            filtered[slot] = valid[:3]
        ...
        for i, (dps, build, result) in enumerate(results, 1):
            ...
            stats = result["stats"]
            self.log(
                f"CRIT: {stats['crit']}  DH: {stats['dh']}  DET: {stats['det']}  SPS: {stats['sps']}\n"
            )
...
# ✅ THIS IS WHAT YOUR EXE NEEDS
def main(items_by_slot):
    root = tk.Tk()
    app = App(root, items_by_slot)
    root.mainloop()


# optional: allow running directly without EXE
if __name__ == "__main__":
    # fallback test (empty data so it doesn't crash)
    main({})
